...
{def $redirect = ''}
{if ezhttp_hasvariable( 'EntiLocaliOauthRequest', 'session' )}
    {set $redirect = concat( '?redirect=', ezhttp( 'EntiLocaliOauthRequest', 'session' ) )}
{/if}

{if and( is_set( $hash ), $hash )}

{'Click the following URL to confirm your account'|i18n('design/standard/user/register')}
http://{$hostname}{concat( 'user/activate/', $hash, '/', $object.main_node_id, $redirect )|ezurl(no)}
