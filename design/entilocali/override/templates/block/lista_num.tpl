...
{if $custom_node}
    ...
    {if and( is_set( $custom_node.data_map.classi_filtro ), $custom_node.data_map.classi_filtro.has_content )}        
        {def $_classes = $custom_node.data_map.classi_filtro.content|explode(',')
        ...
        {switch match=$customs.ordinamento}
        {case match='priorita'}
            {set $sort_hash=hash('priority', 'asc')}
        {/case}
        {case match='pubblicato'}
            {set $sort_hash=hash('published', 'desc' )}
        {/case}
        {case match='modificato'}
            {set $sort_hash=hash('modified', 'desc')}
        {/case}
        {case match='nome'}
            {set $sort_hash=hash('name', 'asc')}
        {/case}
        {case}
            {set $sort_hash=hash('published', 'desc' )}
        {/case}
        ...
        <div class="border-content">
            ...
            <div id="x{$block.id}" class="ui-tabs">	
                ...
                {foreach $children as $index => $child}
                    <div id="{$child.name|slugize()}-{$child.node_id}" class="{if $index|ge(3)}no-js-hide {/if}ui-tabs-hide">
                    ...
                    {else}
                        ...
                        {elseif and(is_set($child.data_map.stemma), $child.data_map.stemma.has_content)}
                            <div class="attribute-image no-js-hide">
                                {attribute_view_gui attribute=$child.data_map.stemma image_class=lista_accordion_nocrop}
                            </div>
                        {else}
