{def
    ...
    $virtual_page = $view_parameters.show
    $virtual_link = concat( '/(show)/', $virtual_page )
...
{def $ezfind_curr_first = sum( $first_ts, 1 )|datetime( 'custom', '%Y-%m-%dT%H:%i:%sZ' )
     $ezfind_curr_last = sub( $last_ts, 1 )|datetime( 'custom', '%Y-%m-%dT%H:%i:%sZ' )
}

{def $events_search_hash = hash( 
                        'limit', 100,
     ...
     $events_search = fetch( ezfind, search, $events_search_hash )
     $events = $events_search['SearchResult']
     $url_reload=concat( $node.url_alias, "/(day)/", $temp_today, "/(month)/", $temp_month, "/(year)/", $temp_year, "/(offset)/2", $virtual_link)
     $url_back=concat( $node.url_alias,  "/(month)/", sub($temp_month, 1), "/(year)/", $temp_year, $virtual_link)
     $url_forward=concat( $node.url_alias, "/(month)/", sum($temp_month, 1), "/(year)/", $temp_year, $virtual_link)
}
        ...
        'main_node_only', true(),
         'attribute_filter',
            array( 'or',
                ...
                array( 'and',
                    array( 'event/from_time', '<', $first_ts),
                    array( 'event/to_time', '>', $last_ts)
                )
            )
        ))
...
{if eq($temp_month, 1)}
    {set $url_back=concat( $event_node.url_alias,"/(month)/", "12", "/(year)/", sub($temp_year, 1), $virtual_link)}
{elseif eq($temp_month, 12)}
    {set $url_forward=concat( $event_node.url_alias,"/(month)/", "1", "/(year)/", sum($temp_year, 1), $virtual_link)}
{/if}
...
{while le( $counter, $days )}
    ...
    {if $day_array|contains(concat(' ', $counter, ',')) }
        <a href={concat( $event_node.url_alias, "/(day)/", $counter, "/(month)/", $temp_month, "/(year)/", $temp_year, $virtual_link)|ezurl}>{$counter}</a>
    {else}
        {$counter}
    {/if}
    </td>
...
{foreach $events as $event}
    ...
    {if and(is_set($event.object.data_map.materia),$event.object.data_map.materia.has_content)}
    <span class="ezagenda_keyword">[
    {attribute_view_gui attribute=$event.object.data_map.materia}
    ]</span>
    {/if}
    {if and(is_set($event.object.data_map.abstract), $event.object.data_map.abstract.has_content)}
        <div class="attribute-short">{attribute_view_gui attribute=$event.object.data_map.abstract}</div>
    {elseif and(is_set($event.object.data_map.text), $event.object.data_map.text.has_content)}
