...
{if and( is_set( $node.data_map.subtree ), $node.data_map.subtree.has_content )}
    {set $event_node_id = $node.data_map.subtree.content.main_node_id}
{/if}
...
<div class="attribute-header">
    <h1>{$event_node.data_map.title.content|wash()}</h1>
</div>
...
{foreach $events as $event}
    ...
    {if $event|has_abstract()}
        <div class="attribute-short">{$event|abstract()}</div>
    {/if}
